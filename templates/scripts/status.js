...
async function delete_protein() {
    const protein = document.getElementById("folders");
    const selectedFile = protein.value;
    try {
        const response = await fetch(`${default_url}/delete_protein?file=${encodeURIComponent(selectedFile)}`);
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const data = await response.json();
...
// Modified: Only log if folders isn't empty or "No files found"
async function loadLog() {
    if (document.getElementById("logCheckbox").checked) {
        const select = document.getElementById("folders");
        const protein_name = select.value;
        // If the select is empty or has "No files found" option, do not log
        if (!protein_name || protein_name === "No files found" || select.options.length === 0) {
